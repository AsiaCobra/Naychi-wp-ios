...
public extension UIView {
    ...
    static func animate(views: [UIView],
                        ...
                        initialSpringVelocity velocity: CGFloat = ViewAnimatorConfig.initialSpringVelocity,
                        options: UIView.AnimationOptions = [],
                        completion: (() -> Void)? = nil, group2Views: Bool = false) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
            for (index, view) in views.enumerated() {
                ...
                var delayInterval = Double(index) * animationInterval
                
                if group2Views, index%2 == 1 {
                    delayInterval = Double(index-1) * animationInterval
                }
